...
async function main() {
  ...
  nativeToken.address = (await callWithRetries(deployContract, ["FaucetToken", ["WTEH", "WTEH", 18, expandDecimals(1000, 18)]])).address
  const gmx = await deployContract("GMX", [])
  const gmtSupply = expandDecimals(401 * 1000, 18)
  const gmt = await deployContract("GMT", [gmtSupply])
  addresses = await deployTokens()
  const { vault, usdg, router, vaultPriceFeed, glp, glpManager } = await deployVault(nativeToken)
}
...
async function deployVault(nativeToken) {
  ...
  return { vault, usdg, router, vaultPriceFeed, glp, glpManager }
}
